...
Only the local `nst` wake tunnel uses the marker handshake. Nstance-server atomically writes an opaque revision to `/run/nstance/tunnels/<load-balancer-key>.active`. vmconfig maps that key to the local wake-tunnel service and, only after its provider-specific readiness check succeeds, atomically writes the same revision to the matching `.ready` file. vmconfig removes readiness whenever the service stops, fails, or observes another revision. Both files live under `/run`, so readiness cannot survive a reboot. Nstance does not invoke systemd or interpret tunnel-provider configuration.

Nstance-server reads the local wake tunnel's readiness marker directly and accepts only an exact desired/ready revision match. vmconfig owns the provider-specific readiness test, such as checking both service state and the tunnel client's local readiness endpoint. Production tunnels have no Nstance marker or agent-health field; their VM restoration is established by ordinary agent health and requested upstream TCP readiness rather than by exposing vmconfig tunnel internals to Nstance.

Wake-tunnel ownership follows shard leadership without a distributed handoff protocol. A server losing leadership withdraws its local `.active` marker and stops its wake endpoint. A new leader starts its local wake endpoint before publishing `.active`, then waits for matching `.ready` before making any further destructive transition. An already-sleeping shard may therefore have a brief wake-path interruption during leader failover; this is accepted in preference to durable but complex old/new-owner coordination.
...
Nstance writes generic proxy and file state. For the leader-local wake tunnel only, it writes generic revisioned desired state and consumes the matching local readiness revision. vmconfig owns production tunnel lifecycle and translates local wake intent into provider-specific service configuration and readiness, keeping Cloudflare- or ngrok-specific paths, commands, and probes out of Nstance.
