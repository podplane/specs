...
2. invokes blocking `WakeTenant(listener)` through the shared root-owned, proxy-accessible Unix gRPC control socket;
...
The listener snapshot contains no provider resources or dynamic upstream addresses. AWS uses the configured `proxy_port` as the target-registration port override. A Google frontend's single `port` supplies its frontend, proxy, and target ports; duplicate destination-IP and port selectors are rejected.

Both processes use the shared `pkg/proxy` configuration structs. Each listener value contains only `tenant`, `groups`, `target_port`, `proxy_port`, and optional `destination_ip`. All communication between nstance-server and `nstance-proxy` uses one root-owned, proxy-accessible Unix gRPC control socket. Its versioned service exposes only a versioned initial-and-replacement listener-configuration stream and listener-scoped `WakeTenant`; it carries no secrets, commands, provider-specific configuration, or dynamic membership state.
...
A separate minimal `nstance-tunnel` component supervises only the local `nst` wake tunnel. Nstance-server communicates with it through a separate, strictly permissioned Unix gRPC socket. The versioned control stream sends only named wake-tunnel identity and revision desired state; it cannot specify executable paths, arbitrary arguments, shell commands, secrets, credentials, or provider-specific configuration. The supervisor reports `starting`, `ready`, `failed`, or `stopped`, and stops every leader-owned wake tunnel when its control stream closes. No lifecycle state files are used.

`nstance-tunnel` runs as a dedicated least-privileged user with access only to the tunnel credentials and configuration it needs. It starts only tunnel implementations configured locally by vmconfig, applies process restart and bounded backoff, and performs the implementation-specific readiness check. vmconfig installs, configures, and hardens the component and tunnel clients, but does not reconcile runtime desired state or readiness. Production `knc` tunnels remain intrinsic to vmconfig, unchanged, and have no Nstance lifecycle or readiness contract.

Wake-tunnel ownership follows shard leadership without a distributed handoff protocol. A server losing leadership closes or replaces its supervisor control stream, causing its leader-owned wake endpoint to stop. A new leader requests its local wake endpoint and waits for the matching revision to report ready before making any further destructive transition. An already-sleeping shard may therefore have a brief wake-path interruption during leader failover; this is accepted in preference to durable but complex old/new-owner coordination.
